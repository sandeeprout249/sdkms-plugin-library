...
function config_kek_key(headers, base_url, kek_key_id, kek_key_version, is_transient)
  local response, err

  --Fetch the KEK
  local url = base_url .. '/kms/v2/keys/' .. kek_key_id .. '/versions/' .. kek_key_version .. '/publicKey?outputFormat=BASE64_DER'
...
function perform_byok(headers, base_url, wrapped_key_value, input)
...
function check(input)
  ...
  elseif input.operation == 'import' or input.operation == 'rotate' then
    ...
    if input.kek_key_id == nil or input.kek_key_id == "" then
      return nil, 'input parameter kek_key_id required'
    end
    if input.kek_key_version == nil or input.kek_key_version == "" then
      return nil, 'input parameter kek_key_version required'
    end
    if input.target_key_name == nil or input.target_key_name == "" then
      return nil, 'input parameter target_key_name required'
    end
    if input.target_key_version == nil or input.target_key_version == "" then
      return nil, 'input parameter target_key_version required'
    end
  end
  if input.operation == 'import' then
    ...
    if input.is_exportable == nil or input.is_exportable == "" then
      return nil, 'input parameter is_exportable required'
    end
...
function run(input)
  ...
  if input.operation == 'configure' then
    ...
    if input.operation == 'import' or input.operation == 'rotate' then
      ...
      -- Get KEK and import into DSM
      local config_kek_key_resp = config_kek_key(headers, base_url, input.kek_key_id, input.kek_key_version, is_transient)
      ...
      if input.operation == 'import' then
        ...
        target_key = create_target_key(input.target_key_name, input.target_key_type, input.target_key_size)
      else
      ...
      local response = perform_byok(headers, base_url, wrapped_byok_value, input)
